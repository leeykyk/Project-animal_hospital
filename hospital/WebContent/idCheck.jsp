...
<head>
<meta charset="UTF-8">
<title>아이디 중복 확인(회원)</title>
</head>
<script>
	function init(){
		if(${openInit}) {
			document.getElementById("id").value=opener.document.getElementById("Member_ID").value;
		}
	}
...
<body onload="init()">
	<form action="idCheck.jsp" method="post" name="f">
		<input type="text" name="id" id="id" /> <input type="submit"
			name="idcheck" value="중복확인" id="idcheck">
	</form>
	...
	<sql:query var="rs" dataSource="${conn}">
		select*from hospital_member where member_ID=?
